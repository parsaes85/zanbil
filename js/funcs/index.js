const showIncredibleProducts = async () => {
    ...
    const res = await fetch('https://leverapi.f4rd1n.ir/api/digikala')
    ...
    products.forEach(product => {
        ...
        <div class="swiper-slide select-none bg-white rounded-xl relative group">
            ...
            <div class="p-4">
                <a href="product.html" class="text-xs sm:text-sm md:text-base">${product.title_fa}</a>
                ...
                ${
                    product.price.discount_percent ? `
                    <p class="text-gray-400 line-through font-medium text-[10px] md:text-xs">${product.price.prev_price.toLocaleString()} هزارتومان</p>
                    ` : ""
                }
                <p class="text-darkRed font-medium sm:font-semibold text-xs md:text-sm">${product.price.current_price.toLocaleString()} هزارتومان</p>
            </div>
...
const showBestSellingProducts = async () => {
    ...
    products.forEach(product => {
        ...
        <div class="swiper-slide select-none bg-white rounded-xl relative group">
            ...
            <div class="p-4">
                <a href="product.html" class="text-xs sm:text-sm md:text-base">
                    ${product.title_fa}
                </a>
                <div class="flex mt-3 mb-2">
                    <i class="fa-solid fa-star text-yellow-400 text-xs"></i>
